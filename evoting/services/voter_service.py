...
class VoterService:
    ...
    def register_voter( 
        ...
        for vid, v in self.data_store.get_all_voters().items():
    ...
    def verify_all_pending(self, verified_by: str) -> int:
        ...
        for vid, voter in self.data_store.get_all_voters().items():
            if not voter["is_verified"]:
                voter["is_verified"] = True
                self.data_store.update_voter(vid, voter)
                count += 1
    ...
    def get_unverified_voters(self) -> dict:
        ...
        return {
            vid: v for vid, v in self.data_store.get_all_voters().items()
