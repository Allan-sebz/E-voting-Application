...
class ResultsViews:
    ...
    def view_poll_results(self):
        ...
        for pos in poll["positions"]:
            ...
            for rank, (cid, count) in enumerate(sorted_results, 1):
    ...
    def view_results_by_station(self):
        ...
        closed_polls = {pid: p for pid, p in polls.items() if p["status"] in ["open", "closed"]}
        ...
        for sid in poll.get("station_ids", []):
            ...
            station_votes = [v for v in votes if v["poll_id"] == pid and v.get("station_id") == sid]
            ...
            for pos in poll["positions"]:
                ...
                for v in station_votes:
                    if v["position_id"] == pos["position_id"] and not v["abstained"]:
                        vote_counts[v["candidate_id"]] = vote_counts.get(v["candidate_id"], 0) + 1
                
                for cid, count in sorted(vote_counts.items(), key=lambda x: x[1], reverse=True):
                    cand = candidates.get(cid, {})
    ...
    def view_system_statistics(self):
        ...
        verified = sum(1 for v in voters.values() if v["is_verified"])
        ...
        voters_who_voted = sum(1 for v in voters.values() if len(v.get("has_voted_in", [])) > 0)
        ...
        for sid, station in stations.items():
            count = sum(1 for v in voters.values() if v["station_id"] == sid)
        ...
        male = sum(1 for v in voters.values() if v["gender"] == "M")
        female = sum(1 for v in voters.values() if v["gender"] == "F")
    ...
    def view_voter_turnout(self):
        ...
        for pid, poll in polls.items():
            if poll["status"] in ["open", "closed"]:
                station_voters = sum(1 for v in voters.values() if v["station_id"] in poll.get("station_ids", []) and v["is_verified"])
